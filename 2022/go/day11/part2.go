			...
			for _, item := range monkey.items {
				...
				if monkey.operand == `*` {
					if monkey.operation == 0 {
						...
						worry = item * monkey.operation % mod
					}
				} else {
				...
				if (worry % monkey.divisible == 0) {
					monkeys[monkey.throwTrue].items = append(monkeys[monkey.throwTrue].items, worry)
				} else {
					monkeys[monkey.throwFalse].items = append(monkeys[monkey.throwFalse].items, worry)
				}
			}
